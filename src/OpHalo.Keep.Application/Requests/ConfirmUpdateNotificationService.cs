...
public sealed record ConfirmUpdateNotificationCommand(
    Guid RequestId,
...
/// <summary>
/// Records the owner's confirmation that a posted customer-page update was sent to the customer
/// via Sms or Email, including the mandatory private page link (ADR-451). This is the sole
/// notification attestation — preparing or launching the SMS/email draft never reaches here.
/// </summary>
public sealed class ConfirmUpdateNotificationService(
    ...
    IClock clock)
{
    ...
    public async Task<Result<KeepRequestDetailResult>> ExecuteAsync(
        ConfirmUpdateNotificationCommand command, CancellationToken ct = default)
    {
        // --- Auth stack ---
        ...
        // --- Row authorization scope: any authorized operator role may attempt confirmation;
        // the domain method enforces that the confirming actor matches the durable
        // PendingNotificationPreparedByAccountUserId recorded by PrepareUpdateNotification
        // (ADR-451), failing closed with NotificationConfirmerMismatch otherwise. ---
        ...
        // --- Domain: apply confirmation ---
        var nowUtc = clock.UtcNow;
        var confirmResult = request.ConfirmUpdateNotification(
            command.RelatedUpdateEventId, channel.Value, currentUser.UserId, actorDisplayName, nowUtc);

        if (confirmResult.IsFailure)
            return Result<KeepRequestDetailResult>.Failure(confirmResult.Error);

        var commitResult = await operatePersistence.CommitAsync(request, confirmResult.Value, ct);
        switch (commitResult)
        {
