...
/// <see cref="ScopeNudgeSuggestion"/> enforces in-domain.</summary>
public sealed record ScopeNudgeSuggestionApiCommand(Guid? CatalogItemId, Guid? OfferingAssemblyId);

public sealed record CreateScopeNudgeRuleApiCommand(
    Guid? TriggerCatalogItemId,
    Guid? TriggerOfferingAssemblyId,
    IReadOnlyList<ScopeNudgeSuggestionApiCommand> Suggestions);

public sealed record UpdateScopeNudgeRuleApiCommand(
    Guid RuleId,
    IReadOnlyList<ScopeNudgeSuggestionApiCommand> Suggestions);

/// <summary>One configured suggestion as seen by Owner/Admin configuration — always includes
/// ineligible/inactive targets (build-log/123), marked for repair rather than omitted.</summary>
public sealed record ScopeNudgeSuggestionConfigRow(
    Guid Id,
    int Order,
...
public sealed record ScopeNudgeRuleConfigRow(
    Guid Id,
    Guid? TriggerCatalogItemId,
    Guid? TriggerOfferingAssemblyId,
    string TriggerDisplayName,
    bool TriggerIsEligible,
    IReadOnlyList<ScopeNudgeSuggestionConfigRow> Suggestions);

/// <summary>
/// API-facing orchestration for the Owner/Admin Paired Nudges rule-configuration surface (Session
/// 2, build-log/123): per-rule Create/Update/Delete plus an account-wide list, price-free. Same
/// gate composition and permission as <see cref="QuickScopeActionConfigApiService"/> —
/// <c>PriceBookCatalogManage</c> — since this is catalog configuration, not a field/technician
/// action. Unlike that service, mutation is per-rule CRUD, not whole-set replace
/// (<see cref="IScopeNudgeRulePersistence"/>): <see cref="UpdateAsync"/> replaces only the
...
/// matching build-log/123's explicit contrast with <c>QuickScopeAction</c>'s write-time eligibility
/// gate. Read rows still compute and expose current eligibility so Owner/Admin can identify targets
/// needing repair.
/// </summary>
public sealed class ScopeNudgeRuleConfigApiService(
    IScopeNudgeRulePersistence persistence,
    ...
    IClock clock)
{
    ...
    public async Task<Result<IReadOnlyList<ScopeNudgeRuleConfigRow>>> ListAsync(CancellationToken ct)
    {
        var gate = await AuthorizeAsync(mutation: false, ct);
        if (gate.IsFailure)
            return Result<IReadOnlyList<ScopeNudgeRuleConfigRow>>.Failure(gate.Error);

        var rules = await persistence.ListForAccountAsync(currentUser.AccountId, ct);
        var rows = new List<ScopeNudgeRuleConfigRow>(rules.Count);
        foreach (var rule in rules)
            rows.Add(await ToRowAsync(rule, ct));

        return Result<IReadOnlyList<ScopeNudgeRuleConfigRow>>.Success(rows);
    }

    public async Task<Result<ScopeNudgeRuleConfigRow>> CreateAsync(CreateScopeNudgeRuleApiCommand command, CancellationToken ct)
    {
        var gate = await AuthorizeAsync(mutation: true, ct);
        if (gate.IsFailure)
            return Result<ScopeNudgeRuleConfigRow>.Failure(gate.Error);
        ...
        var created = ScopeNudgeRule.Create(
        ...
        if (created.IsFailure)
            return Result<ScopeNudgeRuleConfigRow>.Failure(created.Error);
        ...
        if (created.Value.TriggerCatalogItemId is { } triggerCatalogItemId)
        {
            ...
            if (exists.IsFailure)
                return Result<ScopeNudgeRuleConfigRow>.Failure(exists.Error);
        }
        ...
        if (commitResult != ScopeNudgeRuleCommitResult.Committed)
            return Result<ScopeNudgeRuleConfigRow>.Failure(ScopeNudgeRuleErrors.DuplicateTrigger);

        var row = await ToRowAsync(created.Value, ct);
        return Result<ScopeNudgeRuleConfigRow>.Success(row);
    }

    public async Task<Result<ScopeNudgeRuleConfigRow>> UpdateAsync(UpdateScopeNudgeRuleApiCommand command, CancellationToken ct)
    {
        var gate = await AuthorizeAsync(mutation: true, ct);
        if (gate.IsFailure)
            return Result<ScopeNudgeRuleConfigRow>.Failure(gate.Error);
        ...
        if (rule is null)
            return Result<ScopeNudgeRuleConfigRow>.Failure(ScopeNudgeRuleErrors.NotFound);
        ...
        if (replaceResult.IsFailure)
            return Result<ScopeNudgeRuleConfigRow>.Failure(replaceResult.Error);
    ...
    public async Task<Result> DeleteAsync(Guid ruleId, CancellationToken ct)
    {
        ...
        if (rule is null)
            return Result.Failure(ScopeNudgeRuleErrors.NotFound);

        await persistence.DeleteAsync(currentUser.AccountId, ruleId, ct);
        return Result.Success();
    }

    /// <summary>Existence-only check (build-log/123): a nudge trigger or suggestion may target an
    ...
    private async Task<Result> CheckTargetExistsAsync(Guid? catalogItemId, Guid? offeringAssemblyId, CancellationToken ct)
    {
        if (catalogItemId is { } id)
        {
            ...
            return detail is null ? Result.Failure(ScopeNudgeRuleErrors.TargetNotFound) : Result.Success();
        }
    ...
    private async Task<ScopeNudgeRuleConfigRow> ToRowAsync(ScopeNudgeRule rule, CancellationToken ct)
    {
        ...
        var suggestionRows = new List<ScopeNudgeSuggestionConfigRow>(rule.Suggestions.Count);
        foreach (var suggestion in rule.Suggestions.OrderBy(s => s.Order))
        {
            ...
            suggestionRows.Add(new ScopeNudgeSuggestionConfigRow(
        ...
        return new ScopeNudgeRuleConfigRow(
    ...
    private async Task<Result> AuthorizeAsync(bool mutation, CancellationToken ct)
    {
        ...
        // Gate 3 — PriceBookCatalogManage (build-log/123): configuration follows the catalog
