    ...
    IClock clock)
{
    ...
    public async Task<Result> ActivateAsync(Guid catalogItemId, Guid expectedVersion, CancellationToken ct)
    {
        var gate = await AuthorizeAsync(ct);
        if (gate.IsFailure)
            return gate;
    ...
    public async Task<Result> InactivateAsync(Guid catalogItemId, Guid expectedVersion, CancellationToken ct)
    {
        var gate = await AuthorizeAsync(ct);
        if (gate.IsFailure)
            return gate;
