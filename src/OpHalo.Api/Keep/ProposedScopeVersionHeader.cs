...
/// <summary>
/// Strict parser for the <c>X-Keep-ProposedScope-Version</c> optimistic-concurrency header (Session
/// 3.3b), matching the existing <c>X-Keep-*-Version</c> contract (ADR-330-335, DEF-074; see
/// <see cref="OfferingAssemblyVersionHeader"/>). Parsing lives at the API edge so
/// Application/Core stay transport-neutral.
///
/// Contract:
...
///   - an absent header returns <see cref="ProposedScopeErrors.ExpectedVersionRequired"/>;
///   - any present-but-unusable value returns <see cref="ProposedScopeErrors.ExpectedVersionInvalid"/>.
///
/// Create does not carry the header.
/// </summary>
public static class ProposedScopeVersionHeader
{
    public const string HeaderName = "X-Keep-ProposedScope-Version";
    ...
    public static Result<Guid> Parse(IHeaderDictionary headers)
    {
        ...
        if (!headers.TryGetValue(HeaderName, out var values))
            return Result<Guid>.Failure(ProposedScopeErrors.ExpectedVersionRequired);
        ...
        if (values.Count != 1)
            return Result<Guid>.Failure(ProposedScopeErrors.ExpectedVersionInvalid);
