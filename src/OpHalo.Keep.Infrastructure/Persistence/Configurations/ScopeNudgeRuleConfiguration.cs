...
internal sealed class ScopeNudgeRuleConfiguration : BaseEntityConfiguration<ScopeNudgeRule>
{
    protected override void ConfigureEntity(EntityTypeBuilder<ScopeNudgeRule> builder)
    {
        builder.ToTable("keep_pricebook_scope_nudge_rules", t => t.HasCheckConstraint(
            "ck_keep_pricebook_scope_nudge_rules_exclusive_trigger",
        ...
        // Alternate key — supports the composite FK from ScopeNudgeSuggestion.ScopeNudgeRuleId,
        // which must reference (AccountId, Id) so a malformed direct write can never pair a rule
        // from one account with a suggestion carrying a different account (build-log/122/123;
        // matches the pattern established for CatalogItem/OfferingAssembly).
        ...
        // API, matching CatalogItem.Aliases / OfferingAssembly.Items.
        builder.Navigation(x => x.Suggestions)
            .UsePropertyAccessMode(PropertyAccessMode.Field);

        // build-log/123: at most one rule per trigger type per account — no account-wide rule cap.
        // Names kept under Postgres's 63-char identifier limit — the original build-log/123-style
        // "ux_keep_pricebook_scope_nudge_rules_account_trigger_..." names silently truncate at the
        // database, which then no longer matches the exact-name catch in
        // EfScopeNudgeRulePersistence.
        ...
        // matching every other reference to those entities.
