...
import { SmsHandoffView } from "./SmsHandoffView";
...
async function fetchHandoff(handoffToken: string): Promise<HandoffState> {
  ...
  try {
    res = await fetch(
      `${apiBase}/keep/share-sms/${encodeURIComponent(handoffToken)}`,
      { cache: "no-store" }
    );
  } catch {
...
export default async function SmsHandoffPage({
  params,
}: {
      ...
      <main className="flex min-h-screen flex-col px-5 py-8" style={canvasStyle}>
        <div className="mx-auto w-full max-w-sm flex-1">
          ...
          <p className="mt-3 text-sm leading-6 text-muted-foreground">
            Return to OpHalo and create a new text link.
          </p>
        </div>
      ...
      <main className="flex min-h-screen flex-col px-5 py-8" style={canvasStyle}>
        <div className="mx-auto w-full max-w-sm flex-1">
          ...
          <p className="mt-3 text-sm leading-6 text-muted-foreground">
            ...
            revoked. Return to OpHalo and create a new text link.
          </p>
        </div>
        <KeepPageFooter className="mt-8" />
      </main>
