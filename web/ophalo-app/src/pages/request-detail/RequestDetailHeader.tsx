...
export function RequestDetailHeader({ onBack, referenceCode, prevId, nextId, onNavigate }: RequestDetailHeaderProps) {
  return (
    <div className="flex items-center gap-2 px-4 py-3 bg-[var(--ophalo-card)] border-b border-[var(--ophalo-border)] shrink-0">
      <button type="button" onClick={onBack} className={`flex items-center gap-1 text-sm text-[var(--ophalo-muted)] hover:text-[var(--ophalo-ink)] -ml-1 transition-colors ${FOCUS_RING}`}>
        <ChevronLeft className="h-4 w-4" />
        Requests
      </button>
      {referenceCode && <span className="text-sm text-[var(--ophalo-muted)] font-mono ml-1">{referenceCode}</span>}
