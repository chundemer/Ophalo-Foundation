...
Pre-Batch-5 there is no `BillingRevision`, so the revision-void clause is a no-op. 4e is the *pre-export,
pre-revision* correction. The preflight must state how the two unify: 4e's supersede-link lifecycle is the substrate that
...
**Layer:** Application + Api. **Families:** 1 (`CreateReplacementAsync`). **Prod (≈7):**
...
`ActualWork.cs` (widen `Supersede` from not-yet-reviewed to the approved pre-export rule before the
endpoint is exposed),
...
**Tests (≈7):** replacement creates a Draft successor with copied facts (incl. zero-line Outcome/note) and no
financial rows; a reviewed, pre-export source may also be replaced and its successor requires normal
re-review; source drops out of review queue/count/eligible reads and its detail read returns
...
When Billing Revisions land, replacement before export must revoke the affected Draft/Ready revision and
force the successor through review/readiness again; exported/Handed Off revisions are immutable and use
the later adjustment path.

### Slice 4e-iii — replacement-copy UI
**Layer:** `web/ophalo-app`. **Prod (≈5):** `ActualWorkReviewCard.tsx` / new `ReplaceVisitForm.tsx`
(reason-required "Correct this visit" action on a live submitted, pre-export visit, including a reviewed
one), `useActualWorkFinancialReview.ts`
...
live non-superseded pre-export submitted work, including reviewed work; prefilled zero-line values editable + reload-persistent;
...
lifecycle and chain rules, Billing Revision's relationship to pre-export replacement, the
