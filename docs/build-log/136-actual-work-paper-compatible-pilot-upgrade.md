...
- **4c-iii** — rich field UI: composer per-line performer refinement + `VisitNote` textarea (price-blind); history card shows performer + `VisitNote` read-only. `web/ophalo-app`. **COMPLETE (2026-08-30)** — 6 prod + 5 test; per-line performer is add-time only (no backend sub-slice), existing lines read-only; VisitNote autosaves on blur; history card uses a native `<details>` per visit. Frontend suite 823/823. See the session-log 4c handoff entry.
...
- **4f-ii** — workspace office region: composes the existing `FinancialResolutionForm` / `NoChargeDispositionForm` / review controls + blocker list + totals, capability-gated, line-adjacent. `web/ophalo-app`. **Presentation upgrade COMPLETE (2026-09-01)** — the wide Owner/Admin read-only branch now renders a dedicated two-column financial-review workspace (`ActualWorkFinancialReviewWorkspace.tsx`): white header band (`Actual Work Financial Review — Visit #N`, Pending review / Review complete, submitted metadata), left context rail (Job & Customer Context + Technician Field Notes, recorder resolved via `participants`), right audit surface (four KPI cards with semantic margin tone — ≥15% healthy / 0–<15% thin / <0% negative, soft tinted fill — a Line Item Breakdown table with totals row + stacked small-screen variant + `⚠ Resolve cost` badges, and the review card). Cool `--keep-workspace-canvas` page shell (new token; `--ophalo-canvas` unchanged so Price Book keeps its cream). `Complete internal financial review` is client-disabled on incomplete financials (domain gate still authoritative). Call / Text / Email in the context rail open the shared `LogContactModal` (no `tel:`/`mailto:`). Narrow Request Detail path and non-reviewer `ReadOnlyVisit` unchanged. 6 files; frontend suite 988/988. **Open:** no request-title field distinct from `customerName` in `KeepRequestDetailResult` — deferred to Christian.
