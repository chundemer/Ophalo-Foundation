...
**Status:** 2e.1a through 2e.4 are complete; 2e.5 implementation is complete and awaiting manual
browser verification; next implementation session is 2e.6 preflight (Active-item maintenance)
**Date:** 2026-08-03  
...
| **2e.7 — Lifecycle and operating-speed polish** | Add inactive filter/reactivation, final filter/search/pager behavior, no-standalone-price rendering, filtered no-results states, shortcuts help, and accessible mobile/desktop interaction polish. Replace the creation editor's native category select with one accessible searchable combobox and reuse its selection/search model for the category filter; preserve an explicit No category choice and duplicate-safe category creation. | 2e.4–2e.6 | Reactivation preserves current price pointer; all filter changes reset cursor state; browse/search ordering is stable; keyboard, focus, and screen-reader behavior pass focused tests. Category selection works with a short or long account-owned list without seeded taxonomy or a separate divergent filter control. |
