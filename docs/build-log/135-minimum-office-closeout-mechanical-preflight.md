...
12. **Component-by-component resolution supersession** — the effective resolution state is computed
    per component, not per row: the latest row with a non-null resolved sell price wins for sell
    price, the latest row with a non-null resolved direct cost wins for direct cost, and each
    component retains its own provenance. A newer cost-only resolution must not erase an earlier
    resolved sell price. §4 Batch 2, §5 proof 2, and the Batch 3a-ii projection contract.
13. **Batch 2 append seam** — `IActualWorkFinancialResolutionPersistence` gains `AddResolutionAsync`
    / `AddDispositionAsync` in this batch (not just the two read methods): account-scoped, staging
    the immutable entity into the caller's unit of work with no internal transaction, so Batches
    3a-ii / 3b-i compose them inside their load/version/review-state boundary instead of reaching
    into EF or opening a second persistence path.
...
**Layer:** Infrastructure + Foundation migration + Application persistence seam (read + transactional
append; no DI, no mutation-handler family — the append methods only stage entities into the caller's
unit of work). **Families:** 0.
...
4. `src/OpHalo.Keep.Application/PriceBook/IActualWorkFinancialResolutionPersistence.cs` — read
   methods **and** the transactional append seam, so Batch 3a-ii / 3b-i do not reach into EF or
   invent a second persistence path:
   - `GetResolutionsForVisitAsync(accountId, actualWorkId, ct)` — every resolution row for the
     visit, newest-first, for the effective-state projection.
   - `GetDispositionsForVisitAsync(accountId, actualWorkId, ct)` — every disposition row for the
     visit, newest-first.
   - `AddResolutionAsync(ActualWorkLineFinancialResolution resolution, ct)` — append one immutable
     row. Account-scoped (the entity carries `AccountId`); participates in the caller's ambient
     transaction / concurrency boundary (no `BeginTransactionAsync` inside — Batch 3a-ii loads the
     visit, checks `ReviewedAtUtc`/version, and commits the unit of work).
   - `AddDispositionAsync(ActualWorkOfficeFinancialDisposition disposition, ct)` — same contract
     for the visit-level disposition (Batch 3b-i consumer).
   The append methods stage the entity and leave `SaveChangesAsync` to the caller's transaction;
   they do not open or commit one themselves.
5. `src/OpHalo.Keep.Infrastructure/Persistence/EfActualWorkFinancialResolutionPersistence.cs` —
   `account_id`-filtered reads; the two append methods `Add` to the tracked set without saving.
   Class lands here; **DI registration is deferred to Batch 3a-ii** (first consumer).
...
same-account line from a *different* visit is rejected); and **component-by-component effective
resolution**:
- Two rows, same line, both supplying sell price → the read's effective sell price is the newer;
  the older row is retained.
- Row A (older) supplies **only** sell price; row B (newer) supplies **only** direct cost → the
  effective state is A's sell price **and** B's direct cost. A newer cost-only resolution must not
  erase an earlier resolved sell price.
- Each component's effective value carries its own provenance row
  (`ResolvedByAccountUserId`, `Basis`, `Reason`, `ResolvedAtUtc`) — sell price from A, direct cost
  from B in the mixed case.
- A component with no supplying resolution row stays unresolved (still a blocker).

The append seam is exercised here too: `AddResolutionAsync` / `AddDispositionAsync` inside a
caller-owned transaction, with the row visible to a subsequent read only after the caller's
`SaveChangesAsync`.
...
2. `IActualWorkFinancialResolutionPersistence.cs` — **edit**: add the transactional orchestrator
   ...
   The Batch 2 `AddResolutionAsync` staging method is unchanged; this orchestrator composes it.
3. `EfActualWorkFinancialResolutionPersistence.cs` — **edit**: transactional mutation. `BeginTransaction`
   ...
   **`ReviewedAtUtc != null` → `VisitAlreadyReviewed`** (D5) → line not found on visit → the
   targeted component's snapshot on the line is already non-null → stage the resolution row via the
   Batch 2 `AddResolutionAsync` (no separate insert path) → bump `visit.ConcurrencyVersion` (keeps
   the review card's expected version coherent) → `SaveChanges` (catch
   `DbUpdateConcurrencyException` → `VersionMismatch`) → commit. Returns the new visit
   `ConcurrencyVersion`.
4. `ActualWorkFinancialReadApiService.cs` — **edit** (one file: it holds `ActualWorkFinancialProjection`
   **and** the `ActualWorkFinancialLineEntry` / `ActualWorkFinancialDetailResult` records). Load the
   visit's resolutions, compute the **effective** value **per component independently** — the
   latest row with a non-null sell price for sell price, the latest row with a non-null direct cost
   for direct cost (`ResolvedAtUtc DESC, Id DESC`), each with its own provenance — fold into
   `ComputeVisitTotals` / `ToLineEntry`; a line is
...
| **2. Effective financial-resolution supersession (component-by-component)** | `ActualWorkLineFinancialResolution` — append-only, **no unique constraint**. Read model computes the effective state **per component independently**: the latest row with a non-null `resolved_unit_sell_price` (`resolved_at_utc DESC, id DESC`) wins for sell price; the latest row with a non-null `resolved_unit_standard_expected_direct_cost` wins for direct cost. Each component keeps its own provenance row; a newer cost-only resolution never erases an earlier resolved sell price. A component with no supplying row stays a blocker. New rows are rejected once `ReviewedAtUtc != null` (D5). DB proof: (a) two sell-price rows → newer effective, older present; (b) older sell-price-only + newer cost-only → both components resolved from their own rows. | 1 / 2 / 3a-ii |
