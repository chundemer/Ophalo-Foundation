...
2. **GAP-038 — in-product feedback + Help & Updates loop.** Discovery ADR locked: [ADR-500](decisions/ADR-500-in-product-feedback-and-help-updates-loop.md). No migration on the content/awareness path: founder-maintained remote JSON (`entries[]` + `guides[]`) via an authenticated, schema-validating, last-known-good `GET /api/v1/updates` proxy — publish known issues, changes, and guides (with inline captioned screenshots, allowlisted asset origin) with no deploy. Client-computed localStorage watermark drives a `--ophalo-accent` dot on the account-menu trigger + "· N new" on a new all-roles `#/help` row; one self-quieting dismissible Requests-list banner for `highlight` entries. `#/help` is a single sectioned scroll (Known issues → Updates → Coming soon → Guides), light-markdown via a sanitising renderer. `POST /api/v1/feedback` to the private founder channel (no customer PII / broad logs); failed delivery persists to a durable store (minimal `feedback` table permitted) — never a fake "sent". All authenticated-shell only. Sequenced here so change announcements land in-product before the heavy change cadence of the coming month. Not a pilot gate. Next: implementation build-log (remote source, asset origin, renderer, failed-delivery mechanism, visual values) after GAP-054 commits; splits into feed+awareness+page / friction if over the batch gate.
