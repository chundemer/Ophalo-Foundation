...
/// <summary>
/// Locks <see cref="ScopeNudgeRule.Create"/> and <see cref="ScopeNudgeRule.ReplaceSuggestions"/>'s
/// in-domain invariants (build-log/122, build-log/123): exactly one trigger target, and a
/// validated 1–3 suggestion set. Suggestion set-level invariants (count bound, distinct order,
/// distinct target) are <see cref="ScopeNudgeSuggestionSetValidatorTests"/>, not here.
/// </summary>
public class ScopeNudgeRuleTests
{
    ...
    {
        var result = ScopeNudgeRule.Create(AccountId, CatalogItemId, null, OneSuggestion(), Actor);
    ...
    {
        var result = ScopeNudgeRule.Create(AccountId, null, OfferingAssemblyId, OneSuggestion(), Actor);
    ...
    {
        var result = ScopeNudgeRule.Create(AccountId, null, null, OneSuggestion(), Actor);

        Assert.True(result.IsFailure);
        Assert.Equal(ScopeNudgeRuleErrors.TriggerTargetRequired, result.Error);
    }

    [Fact]
    public void Create_with_both_trigger_targets_fails()
    {
        var result = ScopeNudgeRule.Create(AccountId, CatalogItemId, OfferingAssemblyId, OneSuggestion(), Actor);

        Assert.True(result.IsFailure);
        Assert.Equal(ScopeNudgeRuleErrors.TriggerTargetMustBeExclusive, result.Error);
    }
    ...
    public void Create_with_three_suggestions_orders_them_sequentially()
    {
        ...
        var result = ScopeNudgeRule.Create(AccountId, CatalogItemId, null, suggestions, Actor);
    ...
    {
        var result = ScopeNudgeRule.Create(AccountId, CatalogItemId, null, [], Actor);

        Assert.True(result.IsFailure);
        Assert.Equal(ScopeNudgeRuleErrors.SuggestionCountOutOfRange, result.Error);
    }
    ...
    {
        var result = ScopeNudgeRule.Create(AccountId, CatalogItemId, null, [(null, null)], Actor);

        Assert.True(result.IsFailure);
        Assert.Equal(ScopeNudgeRuleErrors.SuggestionTargetRequired, result.Error);
    }

    [Fact]
    public void Create_with_a_suggestion_targeting_both_types_fails()
    {
        var result = ScopeNudgeRule.Create(
            AccountId, CatalogItemId, null, [(SuggestedCatalogItemId, SuggestedOfferingAssemblyId)], Actor);

        Assert.True(result.IsFailure);
        Assert.Equal(ScopeNudgeRuleErrors.SuggestionTargetMustBeExclusive, result.Error);
    }
    ...
    public void Create_with_duplicate_suggestion_targets_fails()
    {
        ...
        Assert.True(result.IsFailure);
        Assert.Equal(ScopeNudgeRuleErrors.DuplicateSuggestionTarget, result.Error);
    }

    [Fact]
    public void ReplaceSuggestions_swaps_the_full_set_and_leaves_the_trigger_unchanged()
    {
        var rule = ScopeNudgeRule.Create(AccountId, CatalogItemId, null, OneSuggestion(), Actor).Value;
    ...
    public void ReplaceSuggestions_with_an_invalid_proposed_set_leaves_existing_suggestions_untouched()
    {
        ...
        Assert.Equal(ScopeNudgeRuleErrors.SuggestionCountOutOfRange, replaceResult.Error);
        Assert.Single(rule.Suggestions);
        Assert.Equal(SuggestedCatalogItemId, rule.Suggestions.Single().SuggestedCatalogItemId);
    }
}
