...
/// <summary>
/// HTTP integration tests for the Actual Work Owner/Admin nudge rule-configuration contract
/// (build-log/129, 5d-ii-b):
///   GET/POST /keep/pricebook/actual-work-nudge-rules            (Owner/Admin, PriceBookCatalogManage)
///   PUT/DELETE /keep/pricebook/actual-work-nudge-rules/{ruleId}
///
...
/// </summary>
public sealed class ActualWorkNudgeRuleApiTests : IClassFixture<KeepApiWebFactory>, IAsyncLifetime
{
    private readonly KeepApiWebFactory _factory;

    public ActualWorkNudgeRuleApiTests(KeepApiWebFactory factory)
    {
        _factory = factory;
    }
    ...
    public async Task Create_AdminWithEntitlement_PersistsAndReturnsPriceFreeRow()
    {
        ...
        var response = await AuthRequest(ownerCookie).PostAsJsonAsync(
            "/keep/pricebook/actual-work-nudge-rules",
            new
            {
        ...
        Assert.Single(db.Set<ActualWorkNudgeRule>().Where(x => x.AccountId == accountId));
    }
    ...
    public async Task Create_UnknownTriggerTarget_Returns404WithTargetNotFound()
    {
        ...
        var body = await response.Content.ReadFromJsonAsync<JsonElement>();
        Assert.Equal("ActualWorkNudgeRule.TargetNotFound", body.GetProperty("code").GetString());
    }
    ...
    public async Task Create_DuplicateTrigger_Returns409()
    {
        ...
        Assert.Equal("ActualWorkNudgeRule.DuplicateTrigger", body.GetProperty("code").GetString());
    }
    ...
    public async Task Create_BothTriggerFieldsPresentOneUnknown_ReturnsExclusivityDomainError()
    {
        ...
        var body = await response.Content.ReadFromJsonAsync<JsonElement>();
        Assert.Equal("ActualWorkNudgeRule.TriggerTargetMustBeExclusive", body.GetProperty("code").GetString());
    }
    ...
    public async Task Create_BothSuggestionFieldsPresentOneUnknown_ReturnsExclusivityDomainError()
    {
        ...
        Assert.Equal(HttpStatusCode.BadRequest, response.StatusCode);
        var body = await response.Content.ReadFromJsonAsync<JsonElement>();
        Assert.Equal("ActualWorkNudgeRule.SuggestionTargetMustBeExclusive", body.GetProperty("code").GetString());
    }
    ...
    public async Task List_IncludesRules_GateDeniesOperator()
    {
        ...
        var listResponse = await AuthRequest(ownerCookie).GetAsync("/keep/pricebook/actual-work-nudge-rules");
        ...
        var operatorCookie = await GetCookieAsync(operatorId, accountId);
        var deniedResponse = await AuthRequest(operatorCookie).GetAsync("/keep/pricebook/actual-work-nudge-rules");
    ...
    public async Task Update_ReplacesSuggestionsOnly_IgnoresTriggerFields()
    {
        ...
        var updateResponse = await AuthRequest(ownerCookie).PutAsJsonAsync(
            $"/keep/pricebook/actual-work-nudge-rules/{ruleId}",
            new
            {
    ...
    public async Task Update_UnknownRuleId_Returns404()
    {
        ...
        var response = await AuthRequest(ownerCookie).PutAsJsonAsync(
            $"/keep/pricebook/actual-work-nudge-rules/{Guid.NewGuid()}",
            new { suggestions = new[] { new { catalogItemId = suggestionId, offeringAssemblyId = (Guid?)null } } });

        Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
        var body = await response.Content.ReadFromJsonAsync<JsonElement>();
        Assert.Equal("ActualWorkNudgeRule.NotFound", body.GetProperty("code").GetString());
    }
    ...
    public async Task Delete_RemovesRuleAndSuggestions()
    {
        ...
        var deleteResponse = await AuthRequest(ownerCookie).DeleteAsync($"/keep/pricebook/actual-work-nudge-rules/{ruleId}");
        Assert.Equal(HttpStatusCode.NoContent, deleteResponse.StatusCode);

        await using var scope = _factory.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<OpHaloDbContext>();
        Assert.Empty(db.Set<ActualWorkNudgeRule>().Where(x => x.Id == ruleId));
        Assert.Empty(db.Set<ActualWorkNudgeSuggestion>().Where(x => x.ActualWorkNudgeRuleId == ruleId));
    }
    ...
    public async Task Delete_UnknownRuleId_Returns404()
    {
        ...
        var response = await AuthRequest(ownerCookie).DeleteAsync($"/keep/pricebook/actual-work-nudge-rules/{Guid.NewGuid()}");
    ...
    private async Task<(Guid AccountId, Guid OwnerAccountUserId, string OwnerCookie)> SeedAccountAsync(string slug)
    {
        ...
        var result = new AccountProvisioningService().CreateVerified(
            ...
            name: "Owner",
            businessName: $"Actual Work Nudge Rule Test Co {slug}",
