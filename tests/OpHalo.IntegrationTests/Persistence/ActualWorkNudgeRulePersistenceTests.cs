...
/// <summary>
/// Proves the per-rule <see cref="ActualWorkNudgeRule"/> persistence seam against real PostgreSQL
/// (5d-ii-a2, build-log/129): duplicate-trigger uniqueness, cascade delete of suggestion rows,
/// cross-account tenant isolation on the composite parent FK, and the <c>ReplaceSuggestions</c> +
/// <c>SaveAsync</c> round-trip — same coverage shape as <c>ScopeNudgeRulePersistenceTests</c>.
/// </summary>
[Collection("Postgres")]
public sealed class ActualWorkNudgeRulePersistenceTests : IClassFixture<PostgresFixture>, IAsyncLifetime
{
    ...
    public ActualWorkNudgeRulePersistenceTests(PostgresFixture fixture) => _fixture = fixture;
    ...
    public async Task InitializeAsync()
    {
        ...
        (AccountId, OwnerId) = await SeedAccountAsync(ctx, "Test Business", "owner@aw-nudge-rule.example.com");
        (OtherAccountId, _) = await SeedAccountAsync(ctx, "Other Business", "owner2@aw-nudge-rule.example.com");
    }
    ...
    public async Task CreateAsync_with_a_valid_rule_commits_and_is_readable_by_id()
    {
        ...
        var persistence = new EfActualWorkNudgeRulePersistence(ctx);

        var rule = ActualWorkNudgeRule.Create(AccountId, triggerId, null, [(suggestedId, null)], OwnerId).Value;
        var result = await persistence.CreateAsync(rule, CancellationToken.None);

        Assert.Equal(ActualWorkNudgeRuleCommitResult.Committed, result);
    ...
    public async Task CreateAsync_with_a_duplicate_catalog_item_trigger_returns_DuplicateTrigger()
    {
        ...
        var firstRule = ActualWorkNudgeRule.Create(AccountId, triggerId, null, [(suggestedId, null)], OwnerId).Value;
        await persistence.CreateAsync(firstRule, CancellationToken.None);

        var secondRule = ActualWorkNudgeRule.Create(AccountId, triggerId, null, [(otherSuggestedId, null)], OwnerId).Value;
        var result = await persistence.CreateAsync(secondRule, CancellationToken.None);

        Assert.Equal(ActualWorkNudgeRuleCommitResult.DuplicateTrigger, result);
    }
    ...
    public async Task CreateAsync_with_a_duplicate_offering_assembly_trigger_returns_DuplicateTrigger()
    {
        ...
        var firstRule = ActualWorkNudgeRule.Create(AccountId, null, assemblyId, [(suggestedId, null)], OwnerId).Value;
        await persistence.CreateAsync(firstRule, CancellationToken.None);

        var secondRule = ActualWorkNudgeRule.Create(AccountId, null, assemblyId, [(otherSuggestedId, null)], OwnerId).Value;
        var result = await persistence.CreateAsync(secondRule, CancellationToken.None);
    ...
    public async Task ReplaceSuggestions_then_SaveAsync_persists_the_new_suggestion_set()
    {
        ...
        var writePersistence = new EfActualWorkNudgeRulePersistence(ctx);
        var rule = ActualWorkNudgeRule.Create(AccountId, triggerId, null, [(originalSuggestion, null)], OwnerId).Value;
        await writePersistence.CreateAsync(rule, CancellationToken.None);

        await using var readCtx = CreateContext();
        var readPersistence = new EfActualWorkNudgeRulePersistence(readCtx);
        ...
        var verifyPersistence = new EfActualWorkNudgeRulePersistence(verifyCtx);
    ...
    public async Task DeleteAsync_cascades_to_suggestion_rows()
    {
        ...
        var remainingSuggestions = await ctx.Set<ActualWorkNudgeSuggestion>()
            .CountAsync(x => x.ActualWorkNudgeRuleId == rule.Id, CancellationToken.None);
    ...
    public async Task ListForAccountAsync_is_scoped_to_the_account()
    {
        ...
        var ownRule = ActualWorkNudgeRule.Create(AccountId, ownTrigger, null, [(ownSuggestion, null)], OwnerId).Value;
        await persistence.CreateAsync(ownRule, CancellationToken.None);
        var otherRule = ActualWorkNudgeRule.Create(OtherAccountId, otherTrigger, null, [(otherSuggestion, null)], OwnerId).Value;
    ...
    public async Task Composite_parent_FK_rejects_a_suggestion_whose_account_does_not_match_its_rules_account()
    {
        ...
        var persistence = new EfActualWorkNudgeRulePersistence(ctx);
        var rule = ActualWorkNudgeRule.Create(AccountId, triggerId, null, [(await InsertCatalogItemAsync(ctx, AccountId), null)], OwnerId).Value;
        await persistence.CreateAsync(rule, CancellationToken.None);

        var ex = await Assert.ThrowsAsync<PostgresException>(() => ctx.Database.ExecuteSqlInterpolatedAsync($"""
            INSERT INTO keep_pricebook_actual_work_nudge_suggestions
                (id, account_id, actual_work_nudge_rule_id, "order", suggested_catalog_item_id,
    ...
    public async Task Database_check_constraint_rejects_a_rule_row_with_neither_trigger_target()
    {
        ...
        var ex = await Assert.ThrowsAsync<PostgresException>(() => ctx.Database.ExecuteSqlInterpolatedAsync($"""
            INSERT INTO keep_pricebook_actual_work_nudge_rules
    ...
    public async Task Database_check_constraint_rejects_a_suggestion_row_with_both_targets()
    {
        ...
        var persistence = new EfActualWorkNudgeRulePersistence(ctx);
        var rule = ActualWorkNudgeRule.Create(AccountId, triggerId, null, [(suggestedCatalogItemId, null)], OwnerId).Value;
        await persistence.CreateAsync(rule, CancellationToken.None);
